...
The `cross-rs` Docker image for compositor compilation has been rebuilt on **Debian Trixie** (glibc 2.41) to fix build script compatibility with CachyOS/Arch hosts (glibc 2.43). The older cross-rs default image (Ubuntu Xenial, glibc 2.31) caused `memoffset` build script failures due to glibc version mismatch.

The Docker image (`compositor/cross/Dockerfile`) includes arm64 cross-compilation libraries:
...
- **README.md** — Architecture diagram updated with Smithay 0.7 trait names, server-side decorations, binary size 1.5MB
- **GUIDE.md** — Build instructions updated with Debian Trixie cross-compilation details
- **ARCHITECTURE.md** — Full compositor section rewrite: Smithay 0.7 trait implementations, cross-compilation setup, memory budget updated
- **RELEASE_NOTES** — New v4.3.1 release notes

---

## Also Includes (from v4.3.0)

### Jellyfin Media Player — Built From Source

Jellyfin Desktop (Qt5) is available directly on the Cardputer Zero. Watch Jellyfin server content at 1080P on an external HDMI monitor.
...
- Custom mpv.conf optimized for ARM playback
...
The Cardputer Zero can connect directly to the Unit C6L via BLE 4.2→5.0, bypassing the MonsterC5 hub entirely. This enables wireless Zigbee/Thread scanning and Meshtastic meshchat without any cables.
